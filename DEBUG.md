...
状态：**STM32 I2C 错误后无法恢复的代码根因已确认；首个错误标志待采集**
...
- 当前 `comm_cmd_error` 只保存“发生过错误”，没有锁存 `hi2c1.ErrorCode`、SR1 或
  恢复次数。因此已确认永久失联的非恢复根因，但无法从现有日志确定最初触发的是
  BERR、OVR 还是其他错误标志。
...
- 固件在 UART 成组发送期间仍会短暂暂停 I2C listen，因此是否消除 10 Hz 压力下
  的 `Errno 121` 必须烧录后验证。
- 21:08 后“错误发生后为何一直不恢复”已由上述 STM32 HAL 路径解释。仍需锁存首个
  错误标志，判断最初是总线时序/电气问题触发 BERR，还是接收不及时触发 OVR。
