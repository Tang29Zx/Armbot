...
- 松开 RT 也会立即用当前真实反馈发送保持命令，避免最后一个更闭合的旧目标继续在
  舵机内部执行；检测到接触后，在 RT 松开前抑制继续闭合，但 LT 打开保持可用。
...
- 新增回归测试在修改前分别复现“接触后不发送保持”和“命令目标覆盖反馈”；修改后
  覆盖接触保持、RT 释放保持、接触后停止继续加压，以及无实际位移不得误判。
- `action_interfaces + action_pkg` 隔离构建通过；`colcon test-result --verbose` 为
  71 tests、0 failures、0 errors、1 个既有 copyright skip。一次把功能测试和系统
  flake8 放在同一 pytest 进程时，flake8 并行 worker 在 fork 阶段发生工具段错误；
  清理其遗留 worker 后，64 项功能测试、单进程 flake8 以及正式 `colcon test` 均
  重新通过。尚未部署到 RDK 执行机械臂动作，因此仍需用户验收。
