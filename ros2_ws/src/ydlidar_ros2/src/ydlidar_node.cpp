...
int main(int argc, char *argv[]) {
  ...
  port = node->declare_parameter("port", port);

  frame_id = node->declare_parameter("frame_id", frame_id);

  list = node->declare_parameter("ignore_array", list);

  baudrate = node->declare_parameter("baudrate", baudrate);

  samp_rate = node->declare_parameter("samp_rate", samp_rate);

  resolution_fixed = node->declare_parameter("resolution_fixed", resolution_fixed);

  m_singleChannel = node->declare_parameter("singleChannel", m_singleChannel);

  auto_reconnect = node->declare_parameter("auto_reconnect", auto_reconnect);

  reversion = node->declare_parameter("reversion", reversion);

  m_isToFLidar = node->declare_parameter("isToFLidar", m_isToFLidar);

  angle_max = node->declare_parameter("angle_max", angle_max);

  angle_min = node->declare_parameter("angle_min", angle_min);

  max_range = node->declare_parameter("max_range", max_range);

  min_range = node->declare_parameter("min_range", min_range);

  frequency = node->declare_parameter("frequency", frequency);
